...
#credit: https://www.perplexity.ai/search/28afb000-2858-4b2d-aa38-7621dc2cd3e8?s=u
```
...
ggplot(data = gapminder_summary_2007,
       mapping = aes(x = continent,
                     y = count)) +
  geom_col() 
#credit: https://www.perplexity.ai/search/17f17a6e-ef49-40e3-985c-66a0b42e7e32?s=u
```
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here: Geom_bar has built into it the default of counting the number of cases for a categorical variable
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function. geom_histogram also defaults to counting the number of cases, but for a given range of values on the x-axis, and with the segmentation by categorical variable as defined by fill

7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe: breaks determines the range of the x axis as well as the width of each bar in the histogram, in this case, years of life expectancy.
